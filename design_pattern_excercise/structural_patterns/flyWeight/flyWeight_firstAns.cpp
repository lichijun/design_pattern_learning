...
/**
 * 基本正确，有两个问题。
 * 1. TreeType的GetName、GetColoar、GetTexture方法最好改成常成员函数，这样可以被const对象调用
 * 2. 享元工厂中map的key不合理，不能保证唯一性
*/
...
public:
    ...
    {
        auto searchRes = _treeTypes.find(name);
        ...
        else
        {
            ...
            _treeTypes.insert(std::make_pair(name, pTreeType));
            return pTreeType;
        }
    }
